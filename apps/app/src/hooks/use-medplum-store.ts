...
const mapPatientsToWorklistPatients = (patients: Patient[], tasks: Task[]): WorklistPatient[] => {
...
export function useMedplumStore(): {
  ...
  const [error, setError] = useState<string | null>(null) // Changed type
  const [accessToken, setAccessToken] = useState<string | null>(null)

  // TODO: Mapping should disappear, we should use FHIR resources directly
  const mappedPatients = useMemo(() => mapPatientsToWorklistPatients(patients, tasks), [patients, tasks])

  const mappedTasks = useMemo(() => mapTasksToWorklistTasks(patients, tasks), [patients, tasks])

  const updateResource = <T extends { id?: string }>(currentResources: T[], updatedResource: T): T[] => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        setError(errorMessage) // Store error message as string
      } finally {
        setIsLoading(false)
      }
    }
  ...
  return {
    ...
    isLoading,
    accessToken,
    error, // Now returns string | null instead of Error | null
    addNotesToTask,
    addTaskOwner,
  }
}
